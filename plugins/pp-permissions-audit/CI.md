...
- Fetches the audit script from a pinned tag (`v2.7.1` by default)
...
The template uses `AUDIT_REF: 'v2.7.1'` to pin to a released version. Bump this when you upgrade. Alternatives:
...
steps:
  ...
  - bash: |
      curl -fsSL https://raw.githubusercontent.com/Nerdy-Q/claude-power-pages-plugins/v2.7.1/plugins/pp-permissions-audit/skills/pp-permissions-audit/scripts/audit.py \
           -o /tmp/audit.py
